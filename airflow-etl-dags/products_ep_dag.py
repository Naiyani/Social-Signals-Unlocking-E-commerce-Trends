...
sample_target_table = Table(
    ...
    Column("product_name_length", String(64)),
    Column("product_description_length", String(64)),
    Column("product_photos_qty", String(64)),
    Column("product_weight_g", String(64)),
    Column("product_length_cm", String(64)),
    Column("product_height_cm", String(64)),
    Column("product_width_cm", String(64)),
)
...
def fetch_products_from_api_callable():
    """
...
def load_products_to_db(ti):
    """
    Fetches data from XCom, connects to the target database,
    and loads the data into a table.
    """
    fetched_data_json = ti.xcom_pull(task_ids='fetch_products_from_api_task')
...
with DAG(
    ...
    doc_md="""
    ### API to Database Data Pipeline: product
    This DAG fetches product data from an API using Basic Auth and loads it into a MySQL stage table.
    ...
    fetch_products_from_api_task = PythonOperator(
        task_id='fetch_products_from_api_task',
        python_callable=fetch_products_from_api_callable,
    )

    # Task to load the fetched data into the database
    load_products_to_db_task = PythonOperator(
        task_id='load_product_to_db_task',
        python_callable=load_products_to_db,
        provide_context=True,
    )

    # Define the task dependencies
    fetch_products_from_api_task >> load_products_to_db_task
