...
def fetch_sellers_from_api_callable():
    """
...
def load_sellers_to_db(ti):
    """
    Fetches data from XCom, connects to the target database,
    and loads the data into a table.
    """
    fetched_data_json = ti.xcom_pull(task_ids='fetch_sellers_from_api_task')
...
with DAG(
    ...
    doc_md="""
    ### API to Database Data Pipeline:sellers
    This DAG fetches sellers data from an API using Basic Auth and loads it into a MySQL stage table.
    
     API Endpoint: `/sellers/`  
     Target Table: `sellers`
    """
) as dag:
    # Task to fetch data from the API
    fetch_sellers_from_api_task = PythonOperator(
        task_id='fetch_sellers_from_api_task',
        python_callable=fetch_sellers_from_api_callable,
    )

    # Task to load the fetched data into the database
    load_sellers_to_db_task = PythonOperator(
        task_id='load_sellers_to_db_task',
        python_callable=load_sellers_to_db,
        provide_context=True,
    )

    # Define the task dependencies
    fetch_sellers_from_api_task >> load_sellers_to_db_task
